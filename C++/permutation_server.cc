...
class PermutationServer : public MidasServer {
  ...
  PermutationServer (const string& host, int port, Serialization_e ser,
		     bool dual_socket=true, ArrayDisposition_e dis=AS_LIST) :
...
{
  cerr << "usage: " << string(name) << " [--ser=0|1|2|-2|4] [--sock=1|2] [--arrdisp=0|1|2] [--convert=1] hostname portnumber" << endl;
...
int main (int argc, char *argv[])
{
  ...
  PermutationServer ms(host, port, 
		       serialization, dual_socket, array_disposition);
  if (convert) {
