...
class FDTools_ {
 ...
 public:
  ...
  static void ReadExact (int fd, char* buff, int bytes) 
  {
  ...
  void readExact (char* buff, int bytes) 
  { FDTools_::ReadExact(readFD_, buff, bytes); }


  // Helper, blocking write method
  static void WriteExact (int fd, const char* buff, int bytes) 
  {
  ...
  void writeExact (const char* buff, int bytes) 
  ...
  int peekAChar ()
  {
	...
	checkTimeout_();
	int r = read(readFD_, &buffer_[0], MAX_BUFF);
  ...
  // if there's an error.
  int clientReadExact (char* data, int initial_len)
  {
    int read_so_far   = 0;
    int still_to_read = initial_len;

    // Stuff still in buffer
    if (where_<length_) {
      int buffer_len = length_ - where_;
      int copy_from_buffer = min(initial_len, buffer_len);
    ...
    while (still_to_read) {
      if (empty_retries<0) return EOF;
      checkTimeout_();
      int r = read(readFD_, &data[read_so_far], still_to_read);
      // 0: probably, socket went away, try again a few times just to be sure
      if (r==0) {
	empty_retries--;
	continue;
		      ...
		      int amt_of_data_to_read)
  {
	...
	data_from_socket.append(c);
      ...
      // some of the data.
      int len = data_from_socket.length();
 ...
 protected:
  ...
  char* buffer_;
  int length_;   // 0..length_-1 is valid of buffer
  int where_;    // where is betwen 0 and length_
