...
void Compare (const string& orig, const string& to_be)
{
  ...
  for (int ii=0; ii<int(orig.length()); ii++) {
    if (orig[ii] != to_be[ii]) {
      ErrComp(orig, to_be);
    }
  }
...
int main ()
{
  ...
  for (int ii=0; ii<int(values.length()); ii++) {
    Arr& a = values[ii];
    const Val& key = a[0];
    string val = a[1];
    ...
    int mem_length = mem-dump_memory;
  ...
  const char *filenames[] = { "special1.pythonpi2", "special2.pythonpi2", 
			...
			0 };
  for (int kk=0; filenames[kk]!=0; kk++) {
  ...
  options.append(None);
  {
    ...
    for (int ii=0; ii<int(expect.length()); ii++) {
      char* dump_memory;
      char* mem;
      int mem_length;
	...
	cout << endl;
      ...
      if (int(expect[ii].length()) != mem_length) { 
	cerr << "Different lengths for " << to_do[ii] << expect[ii].length() << " " << mem_length << endl;
	throw 1;
      }
      for (int jj=0; jj<int(expect[ii].length()); jj++) {
  ...
  // NUMPY
  {
    ...
    *mem = '.';
    int mem_length = mem-dump_memory+1;
    Compare(string(dump_memory, mem_length), 
