...
inline int BytesToDumpVal (const Val& t, DumpContext_& dc);
inline int BytesToDumpTab (const Tab& t, DumpContext_& dc, void* memoize_self);
inline int BytesToDumpOTab (const OTab& t, DumpContext_& dc, void* memoize_self);
inline int BytesToDumpTup (const Tup& t, DumpContext_& dc, void* memoize_self);
inline int BytesToDumpBigInt (const int_n& t, DumpContext_& dc, void* memoize_self);
inline int BytesToDumpArr (const Arr& t, DumpContext_& dc, void* memoize_self);
inline int BytesToDumpProxy (const Proxy& t, DumpContext_& dc);
...
int P2TopLevelBytesToDumpVal (const Val& v, ArrayDisposition_e dis,
			      PicklingIssues_e issues)
{ return TopLevelBytesToDump(v, dis, issues); }

int P2BytesToDumpVal (const Val& v, ArrayDisposition_e dis,
			     PicklingIssues_e issues)
...
#define P2PLAINARRAYDUMP(T, FUN) { Array<T>&ap=*(Array<T>*)arr_data; int len=ap.length(); T*d=ap.data(); for (int ii=0;ii<len;ii++) { FUN(d[ii], dc); } }
#define P2PLAINARRAYDUMP2(T, FUN, ARG) { Array<T>&ap=*(Array<T>*)arr_data; int len=ap.length(); T*d=ap.data(); for (int ii=0;ii<len;ii++) { FUN(d[ii], dc, ARG); } }
...
int BytesToDumpArrayAsList (void* arr_data, char subtype, DumpContext_& dc,
			    void* memoize_self)
{
  if (subtype=='Z') {
    Arr*ap=(Arr*)arr_data;
...
inline int BytesToDumpArrayAsList (const Val& v, DumpContext_& dc)
{
...
inline int BytesToDumpArray (void* arr_data, char subtype, DumpContext_& dc,
			     void* memoize_self)
{
  ...
  Arr*ap=(Arr*)arr_data;
  int bytes = ByteLength(subtype) * ap->length() + 26;
  if (dc.array_preamble_dumped) {
    bytes += 2;
  } else {
    dc.array_handle = dc.current_handle++;
...
inline int BytesToDumpArray (const Val& v, DumpContext_& dc)
{
...
inline int BytesToDumpNumericArray (void* arr_data, char subtype, 
				    DumpContext_& dc, void* memoize_self)
{
  ...
  Arr*ap=(Arr*)arr_data;
  int bytes = ByteLength(subtype)*ap->length()+14+2*BytesToDump4ByteInteger();
  if (dc.numeric_preamble_dumped) {
    bytes += 2;
  } else {
    dc.numeric_handle = dc.current_handle++;
...
inline int BytesToDumpNumericArray (const Val& v, DumpContext_& dc)
{
...
inline int BytesToDumpNumPyArray (void* arr_data, char subtype, 
				  DumpContext_& dc, void* memoize_self)
{
  // Assume v is an Array: If it's not a numeric or POD type, we
  // still have to dump as an Array of Vals, lame but will work
  ...
  // Otherwise, POD data
  Arr*ap=(Arr*)arr_data;
  int bytes = ByteLength(subtype)*ap->length()+50+2*BytesToDump4ByteInteger();
...
inline int BytesToDumpNumPyArray (const Val& v, DumpContext_& dc)
{
...
template <class T>
inline int BytesToDumpTable_ (const T& t, DumpContext_& dc, void* memoize_self)
{
  int bytes = 1 + 1 + 1; // EMPTY_DICT, ( SETITEMS
  if (memoize_self) bytes += BytesToMemoizeSelf_(memoize_self, dc);
...
inline int BytesToDumpTup (const Tup& u, DumpContext_& dc, void* memoize_self)
{
  if (dc.compat_) { // Tup is an Arr for backwards compatibility
...
inline int BytesToDumpBigInt (const int_n& u, DumpContext_& dc, void*memoize_self)
{
  if (dc.compat_) {
  ...
  int bytes = 5; // \x8a char  or \x8b 4 chars  ... overestimate
  bytes += u.bytes();
  if (memoize_self) bytes += BytesToMemoizeSelf_(memoize_self, dc);
  return bytes;
}
...
inline int BytesToDumpArr (const Arr& ca, DumpContext_& dc, void* memoize_self)
{
...
inline int BytesToDumpProxy (const Proxy& p, DumpContext_& dc)
{
  int bytes = 0;
...
inline int BytesToDumpVal (const Val& v, DumpContext_& dc)
{
...
#define LOADARRAYHELP(T, SUB) { v.tag='n'; v.subtype=SUB; int len=byte_len/sizeof(T); Array<T>*ap=(Array<T>*)(&v.u.n); new (ap) Array<T>(len); ap->expandTo(len); memcpy(ap->data(), lc.mem, byte_len); }
...
void P2LoadArray(Val& v, LoadContext_& lc, bool saw_memo=false)
{
  ...
  // Array<T>
  int byte_len = loadStringLength_(lc);
...
#define LOADNUMERICHELP(T, SUB) { v.tag='n'; v.subtype=SUB; int len=byte_len/sizeof(T); Array<T>*ap=(Array<T>*)(&v.u.n); new (ap) Array<T>(len); ap->expandTo(len); memcpy(ap->data(), lc.mem, byte_len); }
...
void P2LoadNumeric(Val& v, LoadContext_& lc, bool saw_memo=false)
{
  ...
  case 'l': {
    v.tag='n'; v.subtype='x'; int len=byte_len/sizeof(long); 
...
{
  // Initialize into so always start with empy table
  int capacity = 256; // 2; //1024; // TODO: Too big?
