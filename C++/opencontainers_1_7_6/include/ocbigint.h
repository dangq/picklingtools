...
template<class I, class BI>
inline void MakeBigIntFromBinary (const char* in, int len,
				  BigInt<I, BI>& int_return);
...
class BigInt {
  ...
  friend void MakeBigIntFromBinary<>(const char* in, int len,
				     BigInt& int_return);
...
#define BIGINT_CONST(T) BigInt (T i, Allocator* a=0): impl_( i<0 ? T(-i) : T(i), a) { sign() = i<0 ? 1 : 0; }
#define BIGINT_CONSTU(T) BigInt (T i, Allocator* a=0): impl_(T(i), a) { sign() = 0; }
  ...
  int threeWayCompare (const BigInt& other) const
  {
    ...
    // 2: 0 1    +1
    // 3: 1 1    -mag_cmp
    int where = sign() + (other.sign()<<1); // 0 .. 3
    switch (where) {
  ...
  int_4  sign () const { return impl_.sign(); }
  int_4& sign ()       { return impl_.sign(); }
 ...
 protected:
  ...
  BigInt& digitThing_ (I digit, int isign) 
  {
    if (sign()==isign) {
...
inline real_8 MakeRealFromBigInt (const BigInt<I, BI>& int_thing)
{
  bool int_sign = int_thing.sign(); // may get ruined by op below
...
template<class I, class BI>
inline BigUInt<I,BI> MakeBigUIntFromBigInt (const BigInt<I,BI>& s,Allocator *a) 
{ BigUInt<I, BI> result(s.impl_, a); result.sign()=0; return result; }
