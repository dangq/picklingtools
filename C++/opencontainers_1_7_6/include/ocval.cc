...
#define VALSWITCHME2(T) switch(tag) { case's':return T(u.s);case'S':return T(u.S);case'i':return T(u.i);case'I':return T(u.I);\
case'l':return T(u.l);case'L':return T(u.L);case'x':return T(u.x);case'X':return T(u.X);case 'b':return T(u.b); case'f':return T(u.f);case'd':return T(u.d); \
case'F':return T(u.F.re, u.F.im); case'D':return T(u.D.re, u.D.im); case'q':{int_n*qp=(int_n*)&u.q;return T(real_8(int_8(qp->as())));} case'Q':{int_un*qp=(int_un*)&u.Q;return T(real_8(int_u8(qp->as())));} }
...
OC_INLINE bool operator== (const Val& v1, const Val& v2)
{
  ...
  if (v1.tag=='q') { // int_n types, but with hugers
    int_n* l1 = (int_n*)&v1.u.q;
    if (OC_IS_INT(v2))         { int_8 t2=v2;   return *l1==int_n(t2); }
    else if (OC_IS_UINT(v2))   { int_8 t2=v2;   return *l1==int_un(t2); } 
    else if (OC_IS_REAL(v2))   { return MakeRealFromBigInt(*l1)==real_8(v2); }
    else if (OC_IS_CX(v2))     { real_8 r1 = MakeRealFromBigInt(*l1); complex_16 t1=r1; complex_16 t2=v2; return t1==t2; }
    else if (v2.tag=='b')      { return int_8(v1)==int_8(v2); }
    else if (v2.tag=='Q')      { int_un* l2=(int_un*)&v2.u.Q; return *l1==*l2;}     else                       { return false; }
  } else if (v1.tag=='Q') { // int_un
    int_un* l1 = (int_un*)&v1.u.Q;
    if (OC_IS_INT(v2))         { int_8 t2=v2;   return *l1==int_n(t2); }
    else if (OC_IS_UINT(v2))   { int_u8 tu2=v2; return *l1==int_un(tu2); }
    else if (OC_IS_REAL(v2))   { return MakeRealFromBigUInt(*l1)==real_8(v2); }
    else if (OC_IS_CX(v2))     { real_8 r1 = MakeRealFromBigUInt(*l1); complex_16 t1=r1; complex_16 t2=v2; return t1==t2; } 
    else if (v2.tag=='b')      { return int_8(v1)==int_8(v2); }
    else if (v2.tag=='q')      { int_n* l2=(int_n*)&v2.u.q; return *l1==*l2; } 
    else                       { return false; }
  } 
  if (v2.tag=='q' || v2.tag=='Q') { return v2==v1; } // Reverse
...
{   
  ...
  // Assertion: No proxies involved in compare, so can get value striaght out
  switch (v1.tag) {
  case 's': return v1.u.s<v2.u.s;
  case 'S': return v1.u.S<v2.u.S;
  case 'i': return v1.u.i<v2.u.i;
...
OC_INLINE bool operator< (const Val& v1, const Val& v2)
{
  ...
  if (v1.tag=='q') { // int_n types, but with hugers
    int_n* l1 = (int_n*)&v1.u.q;
    if (OC_IS_INT(v2))         { int_8 t2=v2;   return *l1<int_n(t2); }
    else if (OC_IS_UINT(v2))   { int_u8 t2=v2;   return *l1<int_un(t2); }
    else if (OC_IS_REAL(v2))   { return MakeRealFromBigInt(*l1)<real_8(v2); }
    else if (OC_IS_CX(v2))     { throw runtime_error("Complex types are not ordered"); } // real_8 r1 = MakeRealFromBigInt(*l1); complex_16 t1=r1; complex_16 t2=v2; return t1<t2; }
    else if (v2.tag=='b')      { return int_8(v1)<int_8(v2); }
    else if (v2.tag=='Q')      { int_un*l2=(int_un*)&v2.u.Q; return *l1<*l2; } 
    else                       { return true; }
  } else if (v1.tag=='Q') { // int_un
    int_un* l1 = (int_un*)&v1.u.Q;
    if (OC_IS_INT(v2))         { int_8 t2=v2;   return *l1<int_n(t2); }
    ...
    else if (OC_IS_CX(v2))     { throw runtime_error("Complex types are not ordered"); } // real_8 r1 = MakeRealFromBigUInt(*l1); complex_16 t1=r1; complex_16 t2=v2; return t1<t2; } 
    else if (v2.tag=='b')      { return int_8(v1)<int_8(v2); }
    else if (v2.tag=='q')      { int_n* l2=(int_n*)&v2.u.q; return *l1<*l2; } 
    else                       { return true; }
  } 
  if (v2.tag=='q' || v2.tag=='Q') { return !(v2<v1 || v2==v1); } // Reverse

  if (OC_IS_INT(v1)) {         // int types
    if      (OC_IS_INT(v2))    { return int_8(v1)<int_8(v2); }
    else if (OC_IS_UINT(v2))   { return int_8(v1)<int_8(v2); }
    else if (OC_IS_REAL(v2))   { return real_8(v1)<real_8(v2); }
    else if (OC_IS_CX(v2))     { throw runtime_error("Complex types are not ordered"); } // complex_16 t1=v1; complex_16 t2=v2; return t1<t2; }
    else if (v2.tag=='b')      { return int_8(v1)<int_8(v2); }
    else                       { return true; }
  } else if (OC_IS_UINT(v1)) { // unsigned int types
    if      (OC_IS_INT(v2))    { return int_8(v1)<int_8(v2); }
    ...
    else if (OC_IS_CX(v2))     { throw runtime_error("Complex types are not ordered"); } // return bool(v1)<bool(v2); }
    else if (v2.tag=='b')      { return bool(v1)<bool(v2); }
    else                       { return true; }
...
OC_INLINE ostream& PrintArray (ostream& os, const Array<T>& a)
{
  ...
  if (ArrayOutputOptions == LIKE_NUMERIC) {
    os << "'" << OCTagToNumeric(TagFor((T*)0)) << "')";
  } else {
    os << "dtype=" << OCTagToNumPy(TagFor((T*)0)) << ")";
  }
  return os;
}
...
OC_INLINE Arr::Arr (Allocator* al) : 
  Array<Val>(ARRAY_DEFAULT_CAPACITY, al) 
{ }
OC_INLINE Arr::Arr (int len, Allocator* al) : 
  Array<Val>(len, al) { }
...
OC_INLINE int Val::entries () const
{
  int ent = -1;
  if (isproxy) {
    Proxy* pp=(Proxy*)&(u.P);
