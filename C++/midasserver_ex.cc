...
class MyServer : public MidasServer {
  ...
  MyServer (bool have_server_send_mesg_at_connect,
	    string host, int port, Serialization_e ser, bool dual_socket=true, 
	    ArrayDisposition_e dis = AS_LIST) :
    MidasServer(host, port, ser, dual_socket, dis),
    count_(0),
...
{
  cerr << "usage: " << string(name) << " [--ser=0|1|2|-2|4|5|6|7|8] [--sock=1|2] [--arrdisp=0|1|2] [--convert=1] [--server_send_message_at_connect=0|1] hostname portnumber" << endl;
...
int main (int argc, char *argv[])
{
  ...
  while (ii()) { 
    ...
    else if (key=="--sock")    dual_socket   = (int_4(value)==2);
  ...
  MyServer ms(server_send,
	      host, port, serialization, dual_socket, array_disposition);
  if (convert) {
