...
inline size_t BytesToSerializeList (const OpalTable& t, OVDumpContext_& dc)
{
  ...
  // ... then n vals.
  const int_u4 len = t.surfaceEntries();
...
inline void DeserializeTable (OpalValue& ov, OVLoadContext_& lc)
{
  char*& mem=lc.mem;
...
inline void DeserializeArrays (OpalValue& ov, OVLoadContext_& lc)
{ 
  char*& mem=lc.mem;
  ...
  int bytes = ByteLength(m2ktag)*len;
...
inline void DeserializeString (OpalValue& ov, OVLoadContext_& lc)
{ 
  char*& mem=lc.mem;
...
inline void Deserialize (OpalValue& ov, OVLoadContext_& lc)
{
  ...
  case 'a': DeserializeString(ov, lc); break;
  case 't': DeserializeTable(ov, lc); break;
  case 'n': DeserializeArrays(ov, lc); break;
