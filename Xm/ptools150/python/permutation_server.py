...
class PermutationServer(MidasServer) :
    ...
    def __init__ (self, host, port, ser, dual_socket, dis) :
...
try :
    ...
    print "Usage: python permutation_server.py [--ser=0|1|2|-2] [--sock=1|2] [--arrdisp=0|1|2] host port"
    sys.exit(1)

host = args[0]
port = string.atoi(args[1])
serialization = SERIALIZE_P0  # Not fast, but backwards compatible
dual_socket   = DUAL_SOCKET   # backwards compat, but use false (single)if able
...
a = PermutationServer(host, port, serialization, dual_socket,array_disposition)
a.open()
