...
Array<T>*n=(Array<T>*)&u.n;return *n;}
    ...
    // entity
    OC_INLINE int entries () const;
    inline int length () const { return entries(); }
...
class Arr : public Array<Val> {
 public:
  ...
  OC_INLINE Arr (int expected_len, Allocator*a = 0);
...
// for (int ii=0; ii<some_arr.length(); ii++) {
...
class ArrIt {
 public:
  // Arr to iterate over
  ArrIt (const Array<T>& a): ii_(-1), len_(a.length()), data_((T*)a.data()) { }

  // Plain constructor ... nothing to iterate over
  ArrIt () : ii_(-1), len_(0), data_(0) { }
  ...
  void reset (const Array<T>& c) { ii_=-1; len_=c.length(); data_=c.data(); }
  
 protected:
  mutable T key_;   // If the object is an Arr, then we have to have a "key"
  int ii_;  // where in the Arr
  int len_;
...
class ArrSit {
 public:
  // Arr to iterate over
  ArrSit (const Array<T>& a) : ii_(-1), arp_(a.length())
  {
  ...
  // Plain constructor ... nothing to iterate over
  ArrSit () : key_(0), ii_(-1), arp_() { }

  // Advance the iterator one position.  Returns true if the new
  // position is valid, false otherwise.
  bool next () { return ++ii_ < int(arp_.length()); }
  ...
  // Resets the iterator to iterate over collection c
  void reset (const Array<T>& c) { arp_=&c; ii_=-1; }
