...
const int_4 MEMORY_BOUNDARY=INT_MIN;
...
class SPInfo_ { 
 ...
 public: // protected:
  ...
  SPInfo_* prevContiguous () const 
  {
    ...
    int_4* total_bytes_of_previous = (int_4*) c;
    int_4 bytes = *total_bytes_of_previous;
  ...
  // this returns 0
  int_4 canSatisfy (int bytes_requested, int alignment)
  {
    ...
    int_4 bytes_in_inuse_node = bytes_requested+inuse_overhead;
    {
      // Make sure to correct so that split node will ALSO be aligned
      int_4 where_next_free_node_would_start = 
	((AVLP)this) + bytes_in_inuse_node + sizeof(totalBytes_) + sizeof(pool_);
      int remainder = where_next_free_node_would_start % alignment;
      if (remainder) {
  ...
  SPInfo_* splitNode (int bytes_requested, int alignment)
  {
    ...
    int_4 bytes_in_inuse_node = canSatisfy(bytes_requested, alignment);
    int_4 bytes_in_free_node = -totalBytes_ - bytes_in_inuse_node;
    if (bytes_in_free_node <= int_4(sizeof(SPInfo_))) {
