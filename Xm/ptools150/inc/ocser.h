...
class OCSerializer {
 ...
 public:
  ...
  OCSerializer (int start_capac=SER_START_LEN) :
  ...
  char* peek (int& len) { len = len_; return mem_; }
...
class OCDeserializer {
 ...
 protected:
  ...
  // of memory given
  OC_INLINE void load (OCString* tp);   // Load and create a Str in raw memory
  OC_INLINE void load (Tab* tp);   // Load and create a Tab in raw memory
  OC_INLINE void load (Arr* tp);   // Load and create a Arr in raw memory
  OC_INLINE void load (Proxy* pp); // Load and create a Proxy in raw memory

  template <class T>
  OC_INLINE void load (Array<T>* tp); // Load and create an Array in raw memory
