...
class Array {
  ...
  public:
    ...
    Array (const Array<T>& c, Allocator*a = 0) : 
      ...
      reservedSpace_(c.reservedSpace_),
	...
	PlaceCopyCons_(&data_[ii], c[ii], allocator_);
      }
    }
	...
	PlaceCopyCons_(this, rhs, allocator_);
      }
      return *this;
    }
    ...
    // is set to 0.
    void adoptMemory (T* adopt_me, int cap, int use_new_and_delete=1)
    {
      releaseResources_();
      allocator_ = 0;
      length_ = 0;
	...
	resize(2*capac_);
      ...
      PlaceCopyCons_(&data_[length_], a, allocator_);
      length_++;
    }
	...
	resize(2*capac_);
      ...
      PlaceCopyCons_(&data_[length()], T(), allocator_);
    ...
    {
      size_t count = 0;
      int ii=length()-1;   // Removing from end first faster than front
      while (ii>=0) {
	if (val==data_[ii]) {
	  removeAt(ii);
	  count++;
	...
	arrayError_(i); // Note:  This always throws an exception
      ...
      // Move em over
      int jj=i; int len_minus_1=int(length())-1;
	...
	return;
      ...
      if (allocator_) {
	for (; ii<len; ii++) {
	  PlaceCopyCons_(&new_data[ii], data_[ii], allocator_);
	}
    ...
    void swap (Array<T>& rhs) 
    {
      ...
      OC_NAMESPACED::swap(useNewAndDelete_, rhs.useNewAndDelete_);
      OC_NAMESPACED::swap(reservedSpace_, rhs.reservedSpace_);
    ...
    int_4  reservedSpace () const { return reservedSpace_; } 
    int_4& reservedSpace ()       { return reservedSpace_; } 
  ...
  protected:
    ...
    // 2 means new T[]
    int_4 useNewAndDelete_;

    // Extra pad most of the time, may be useful to keep class data
    // in under 32 bytes
    int_4 reservedSpace_;
